...
def runBLAST(genomeFlag,bitCut,coverCut):
	eval = input("Choose E-value: ")
	for num in genomeFlag:
		num = int(num)
		print("Running BLAST..."+str(num)+"\n")
		system(".\\database\\blastp.exe -query " + files[num-1] + " -out ./database/tmp/blast" + " -db ./database/database -evalue " + str(eval) + " -outfmt 6")	
		of = open('./database/'+str(num),'w')
		f = open('./database/tmp/blast','r')
		genej = None
		for line in f:
			genei = line.split('\t')[0]
			if genei != genej:
				maxBit = float(line.split('\t')[11])
				of.write(line)
			else:
				if float(line.split('\t')[11])/maxBit > float(bitCut) and float(line.split('\t')[2]) > float(coverCut):
					of.write(line)
			genej = line.split('\t')[0]
		f.close()
		of.close()
...
def stage02():
	print("\n\nSTAGE 2: BLAST...\n\n")
	bitCut = input("bit score threshold: ")
	coverCut = input("%cover threshold: ")
	genomeFlag = choseGenomes()
	runBLAST(genomeFlag, bitCut, coverCut)
